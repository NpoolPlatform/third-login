...
func Update(ctx context.Context, in *npool.ThirdParty) (*npool.ThirdParty, error) {
	schema, err := crud.New(context.Background(), nil)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	resp, err := schema.Row(context.Background(), uuid.MustParse(in.GetID()))
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	if resp.BrandName != in.GetBrandName() {
		schema, err = crud.New(context.Background(), nil)
		if err != nil {
			return nil, status.Error(codes.Internal, err.Error())
		}
