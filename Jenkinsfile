  ...
  stages {
    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh (returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists third_login_gateways;"
          ...
          for vhost in `cat cmd/*/*.viper.yaml | grep hostname | awk '{print $2}' | sed 's/"//g' | sed 's/\\./-/g'`; do
            ...
            ./apollo-item-config.sh $APP_ID $TARGET_ENV $vhost database_name third_login_gateways
            cd -
          done
        '''.stripIndent())
      }
    ...
    stage('Release docker image for development') {
      ...
      steps {
        ...
        sh(returnStdout: false, script: '''
          images=`docker images | grep entropypool | grep third-login-gateway | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep third-login-gateway | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
